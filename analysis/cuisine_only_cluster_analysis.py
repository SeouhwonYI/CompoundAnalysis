...
def _normalize_ingredient_token(token: str) -> str:
    token = re.sub(r"\s+", " ", str(token)).strip().lower()
    token = re.sub(r"^[\"'\[\(\{]+|[\"'\]\)\}]+$", "", token).strip()
    return token


def _parse_ingredient_cell(val) -> list[str]:
    ...
    out: list[str] = []
    for item in items:
        tok = _normalize_ingredient_token(item)
        if tok and tok not in {"none", "null", "nan"}:
            out.append(tok)
    return out
    ...
    if rid_col and ing_col:
        for rid, ing in recipes_long[[rid_col, ing_col]].itertuples(index=False):
            if pd.isna(rid) or pd.isna(ing):
                continue
            for token in _parse_ingredient_cell(ing):
                rid2ings[int(rid)].add(token)
...
def load_from_graph_csv(cuisine: str) -> tuple[dict, list[int], dict[int, set[str]]]:
    ...
    if not rid2ings:
        extra_files = [
            GRAPH_ROOT / cuisine / "000_recipe_ingredient_edges.csv",
            GRAPH_ROOT / cuisine / "recipe_ingredient_edges.csv",
            GRAPH_ROOT / cuisine / "recipes_long.csv",
        ]
        for extra in extra_files:
            if not extra.exists():
                continue
            try:
                exdf = pd.read_csv(extra)
            except Exception:
                continue
            rid2ings = _extract_rid2ings_from_df(exdf)
            if rid2ings:
                print(f"[{cuisine}] ingredient fallback loaded from {extra.name}: {len(rid2ings)} recipes")
                break
...
def plot_cuisine_summary(cuisine: str, rids: list[int], p_by_r: dict, rid2ings: dict, mol_id_to_name: dict[int, str], out_dir: Path, top_k_clusters: int, df_rate_ban_threshold: float, verbose_ban: bool) -> dict | None:
    ...
    for ax, cid in zip(panels, top_ids):
        ...
        if not lifts:
            lifts = ingredient_lift_for_cluster_soft(rids, rid2w, cid, rid2ings, min_df=2)
        if not lifts:
            lifts = ingredient_lift_for_cluster_soft(rids, rid2w, cid, rid2ings, min_df=1)
        txt = "\n".join([f"{i+1}. {ing} (lift={lift:.2f})" for i, (ing, lift, *_rest) in enumerate(lifts[:12])])
        if not txt:
            covered = sum(1 for rid in rids if rid in rid2ings and rid2ings.get(rid))
            txt = f"(ingredient lift unavailable\ncovered recipes: {covered}/{len(rids)})"
...
def main() -> None:
    ...
    for cuisine in cuisines:
        ...
        covered = sum(1 for rid in rids if rid in rid2ings_source and rid2ings_source.get(rid))
        print(f"ingredient coverage: {covered}/{len(rids)}")
