...
def load_saved_state() -> tuple[dict, dict, dict]:
    ...
    rid_col = _pick_col(recipes_long, ["rid", "recipe_id"])
    ing_col = _pick_col(recipes_long, ["ingredient", "ingredients"])
...
def node_weights(cluster_nodes: list[int], p_m: dict, mol_id_to_name: dict) -> dict[str, float]:
    ...
    for m in cluster_nodes:
        ...
        if val <= 0:
            continue
        out[mol_id_to_name.get(int(m), f"m{m}")] = float(val)
    return out
...
def plot_cuisine_summary(cuisine: str, rids: list[int], p_by_r: dict, rid2ings: dict, mol_id_to_name: dict[int, str], out_dir: Path, top_k_clusters: int) -> dict | None:
    ...
    ban_mols = {m for m, r in df_rate.items() if r > 0.75}
